...
class ConditionalMMFeatFusionDecoder(nn.Module):
    """A conditional decoder with attention and multimodal feat fusion."""
    def __init__(self, input_size, hidden_size, ctx_size_dict, ctx_name, n_vocab,
                 ...
                 feat_fusion='early_concat', aux_dim=2048, aux_proj_dim=256, 
                 aux_activ='tanh', feat_activ='tanh'):
        super().__init__()
        ...
        self.aux_proj_dim = aux_proj_dim
        ...
        if self.feat_fusion in ['early_concat', 'middle_concat', 'weighted_concat', 'attn', 'bbox_attn']:
            self.aux_proj = FF(self.aux_dim, self.aux_proj_dim, activ=self.aux_activ)
        ...
        if self.feat_fusion in ["attn", "bbox_attn"]:
            self.attn_fusion = HierarchicalAttention([self.aux_proj_dim, self.hidden_size], self.hidden_size, self.hidden_size)

        if self.feat_fusion == 'bbox_attn':
            self.bbox_att = Attention(
                self.aux_proj_dim,
                self.hidden_size,
                transform_ctx=self.transform_ctx,
                mlp_bias=self.mlp_bias,
        ...
        # Create concat + projection
        if self.feat_fusion in ['early_concat', 'weighted_concat']:
            self.concat_proj = FF(self.input_size+self.aux_proj_dim, self.input_size, activ=None)
        elif self.feat_fusion == 'middle_concat':
            self.concat_proj = FF(self.hidden_size+self.aux_proj_dim, self.hidden_size, activ=None)

        # Create decoders
        #if self.feat_fusion in ['early_concat', 'weighted_concat']:
        #    self.dec0 = RNN(self.input_size+self.aux_proj_dim, self.hidden_size)
        #else:
        self.dec0 = RNN(self.input_size, self.hidden_size)

        #if self.feat_fusion == 'middle_concat': 
        #    self.dec1 = RNN(self.hidden_size+self.aux_proj_dim, self.hidden_size)
        #else:
    ...
    def f_next(self, ctx_dict, y, h):
        ...
        x = self.aux_proj(ctx_dict['feats'][0].squeeze(0))

        # Get hidden states from the first decoder (purely cond. on LM)
        if self.feat_fusion == 'early_concat':
             y = torch.cat((y, x), dim=1)
             y = self.concat_proj(y)
        elif self.feat_fusion == 'weighted_concat':
             weights = F.sigmoid(torch.sum(torch.mul(x, y), 1))
             weights_repeated = weights.repeat(x.shape[1]).reshape((x.shape[1], x.shape[0])).transpose(0, 1)
             x = torch.mul(weights_repeated, x)
             y = torch.cat((y, x), dim=1)
             y = self.concat_proj(y)
        ...
        if self.feat_fusion == 'attn':
            ctx_list = [x, txt_z_t]
            alpha_t, z_t = self.attn_fusion(ctx_list, h1.unsqueeze(0))
        elif self.feat_fusion == 'bbox_attn':
            bbox_alpha_t, bbox_z_t = self.bbox_att(
                h1.unsqueeze(0), x)
            ctx_list = [bbox_z_t, txt_z_t]
            alpha_t, z_t = self.attn_fusion(ctx_list, h1.unsqueeze(0))
        ...
        if self.feat_fusion == 'middle_concat':
            z_t = torch.cat((z_t, x), dim=1)
            z_t = self.concat_proj(z_t)


        if not self.training:
            self.history['alpha_txt'].append(txt_alpha_t)
            # print(alpha_t.shape)
            self.history['alpha_hier'].append(alpha_t)
            # print(len(self.history['alpha_hier']))
            if self.feat_fusion == 'bbox_attn':
                self.history['alpha_bbox'].append(bbox_alpha_t)
    ...
    def forward(self, ctx_dict, y):
        ...
        return {'loss': loss, 'history': self.history}
